...
class User_model extends CI_Model
{
	 ...
	 * @return bool
	 */
	public function have_display_name($display_name)
	{
		$query = $this->db->get_where('users', array('display_name'=>$display_name));
		if ($query->num_rows() == 0)
			return FALSE;
		if ($display_name === $query->row()->display_name) // needed (because of utf8_general_ci [ci=case insensitive])
			return TRUE;
		return FALSE;
	}
	 ...
	 * @return bool|string
	 */
	public function add_user($username, $display_name, $email, $password, $role)
	{
		...
		if ($this->have_display_name($display_name))
			return 'User with this display name exists.';
	...
	public function delete_user($user_id)
	{
		...
		if ($this->db->trans_status()) {
			...
			shell_exec("cd {$this->settings_model->get_setting('problems_root')}; rm -r */*/{$username};");
			return TRUE; //success
		}
	...
	public function delete_submissions($user_id)
	{
		...
		foreach ($all_problems as $problem) {
			$submissions_count = $this->db->where(array('problem'=>$problem['id'],'username' => $username))->count_all_results('submissions');
			$this->probleminfo_model->update_total_submissions($problem['id'], $submissions_count);
		}
	 ...
	 * Selected problem
	 *
	 * Returns selected problem by given username
	 * @param $username
	 * @return mixed
	 */
	public function selected_problem($username)
	{
		$query = $this->db->select('selected_problem')->get_where('users', array('username'=>$username));
		if ($query->num_rows() != 1){//logout
			$this->session->sess_destroy();
			redirect('login');
		}
		return $query->row()->selected_problem;
	}
