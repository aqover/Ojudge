...
class Queue_model extends CI_Model
{
	 ...
	 */
	public function in_queue ($username, $problem)
	{
		$query = $this->db->get_where('queue', array('username'=>$username, 'problem'=>$problem));
	...
	public function rejudge($problem_id)
	{
		...
		$submissions = $this->db
			->select('submit_id, username, problem')
			->order_by('submit_id')
			->get_where('submissions', array('problem'=>$problem_id))
			->result_array();
	...
	{
		$problem = $this->problem_model->problem_info($submission['problem']);
		if ($problem['is_upload_only'])
			return;
	 ...
	 */
	public function remove_item($username, $problem, $submit_id)
	{
	...
	public function save_judge_result_in_db ($submission, $type)
	{
		...
		$this->scoreboard_model->update_scoreboard($submission['problem']);
	}
