...
class Submissions extends CI_Controller
{
	...
	public function all()
	{
		...
		$config = array(
			...
			'total_rows' => $this->submit_model->count_all_submissions($this->user->level, $this->user->username, $this->filter_user, $this->filter_problem),
		...
		$submissions = $this->submit_model->get_all_submissions($this->user->level, $this->user->username, $this->page_number, $this->filter_user, $this->filter_problem);
		...
		foreach ($submissions as &$item)
		{
			...
			$item['final_score'] = ceil($item['pre_score']);
		}
	...
	public function the_final()
	{
		...
		$config = array(
			...
			'cur_page' => $this->page_number,
			'total_rows' => $this->submit_model->count_final_submissions($this->user->level, $this->user->username, $this->filter_user, $this->filter_problem),
		...
		if ($config['per_page'] == 0)
			$config['per_page'] = $config['total_rows'];
		$this->load->library('shj_pagination', $config);
		$submissions = $this->submit_model->get_final_submissions($this->user->level, $this->user->username, $this->page_number, $this->filter_user, $this->filter_problem);
		$names = $this->user_model->get_names();
		foreach ($submissions as &$item)
		{
			$item['name'] = $names[$item['username']];
		...
		if($this->form_validation->run())
		{
			...
			if ($type === 'result')
				$file_path = rtrim($this->settings_model->get_setting('problems_root'),'/').
					"/p{$submission['problem']}/user/{$submission['username']}/result-{$submission['submit_id']}.html";
			elseif ($type === 'code')
				$file_path = rtrim($this->settings_model->get_setting('problems_root'),'/').
					"/p{$submission['problem']}/user/{$submission['username']}/{$submission['file_name']}.".filetype_to_extension($submission['file_type']);
			elseif ($type === 'log')
				$file_path = rtrim($this->settings_model->get_setting('problems_root'),'/').
					"/p{$submission['problem']}/user/{$submission['username']}/log-{$submission['submit_id']}";
			else
