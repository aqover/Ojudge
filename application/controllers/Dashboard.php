...
class Dashboard extends CI_Controller
{
	...
	public function index()
	{
		...
		if($this->user->level === 3){
			$path = $this->settings_model->get_setting('problems_root');
			if ( ! file_exists($path))
				array_push($data['errors'], 'The path to folder "problems" is not set correctly. Move this folder somewhere not publicly accessible, and set its full path in Settings.');
