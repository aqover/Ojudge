...
class Install extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->form_validation->run()) {
			...
			// Creating Tables:
			// sessions, submissions, notifications, problems, queue, scoreboard, settings, users
			...
			$this->dbforge->add_field($fields);
			$this->dbforge->add_key(array('submit_id'));
			...
			$fields = array(
				...
				'total_submits'     => array('type' => 'INT', 'constraint' => 11, 'default' => '0'),
			...
			$this->dbforge->add_field($fields);
			$this->dbforge->add_key(array('id'));
			if ( ! $this->dbforge->create_table('problems_info', TRUE))
				show_error("Error creating database table ".$this->db->dbprefix('problems_info'));
			...
			//Add UNIQUE (submit_id, username, problem) constraint
			$this->db->query(
				"ALTER TABLE {$this->db->dbprefix('queue')}
				 ADD CONSTRAINT {$this->db->dbprefix('suap_unique')} UNIQUE (submit_id, username, problem);"
			);
