...
LOG="$PROBLEMPATH/user/$UN/log"; echo "" >>$LOG
function shj_log
{
	if $LOG_ON; then
		echo -e "$@" >>$LOG 
...
if [ "$EXT" = "java" ]; then
	...
	cp $PROBLEMPATH/user/$UN/$FILENAME.java $MAINFILENAME.java
	...
	if [ $EXITCODE -ne 0 ]; then
		...
		echo '<span class="shj_b">Compile Error</span>' >$PROBLEMPATH/user/$UN/result.html
		echo '<span class="shj_r">' >> $PROBLEMPATH/user/$UN/result.html
		#filepath="$(echo "${JAIL}/${FILENAME}.${EXT}" | sed 's/\//\\\//g')" #replacing / with \/
		(cat cerr | head -10 | sed 's/&/\&amp;/g' | sed 's/</\&lt;/g' | sed 's/>/\&gt;/g' | sed 's/"/\&quot;/g') >> $PROBLEMPATH/user/$UN/result.html
		#(cat $JAIL/cerr) >> $PROBLEMPATH/user/$UN/result.html
		echo "</span>" >> $PROBLEMPATH/user/$UN/result.html
		cd ..
...
if [ "$EXT" = "py2" ]; then
	cp $PROBLEMPATH/user/$UN/$FILENAME.py $FILENAME.py
	...
	if [ $EXITCODE -ne 0 ]; then
		...
		shj_log "$(cat cerr | head -10)"
		echo '<span class="shj_b">Syntax Error</span>' >$PROBLEMPATH/user/$UN/result.html
		echo '<span class="shj_r">' >> $PROBLEMPATH/user/$UN/result.html
		(cat cerr | head -10 | sed 's/&/\&amp;/g' | sed 's/</\&lt;/g' | sed 's/>/\&gt;/g' | sed 's/"/\&quot;/g') >> $PROBLEMPATH/user/$UN/result.html
		echo "</span>" >> $PROBLEMPATH/user/$UN/result.html
		cd ..
...
if [ "$EXT" = "c" ] || [ "$EXT" = "cpp" ]; then
	...
	cp $PROBLEMPATH/user/$UN/$FILENAME.$EXT code.c
	...
	if [ $EXITCODE -ne 0 ]; then
		shj_log "Compile Error"
		shj_log "$(cat cerr | head -10)"
		echo '<span class="shj_b">Compile Error<br>Error Messages: (line numbers are not correct)</span>' >$PROBLEMPATH/user/$UN/result.html
		echo '<span class="shj_r">' >> $PROBLEMPATH/user/$UN/result.html
		SHIELD_ACT=false
		if $C_SHIELD_ON; then
			while read line; do
				if [ "`echo $line|cut -d" " -f1`" = "#define" ]; then
					if grep -wq $(echo $line|cut -d" " -f3) cerr; then
						echo `echo $line|cut -d"/" -f3` >> $PROBLEMPATH/user/$UN/result.html
						SHIELD_ACT=true
						break
					fi
				fi
		...
		if ! $SHIELD_ACT; then
			...
			(cat cerr | sed 's/&/\&amp;/g' | sed 's/</\&lt;/g' | sed 's/>/\&gt;/g' | sed 's/"/\&quot;/g') >> $PROBLEMPATH/user/$UN/result.html
		fi
		echo "</span>" >> $PROBLEMPATH/user/$UN/result.html
		cd ..
		rm -r $JAIL >/dev/null 2>/dev/null
...
echo "" >$PROBLEMPATH/user/$UN/result.html
...
for((i=1;i<=TST;i++)); do
	shj_log "\n=== TEST $i ==="
	echo "<span class=\"shj_b\">Test $i</span>" >>$PROBLEMPATH/user/$UN/result.html
	
	touch err
	
	if [ "$EXT" = "java" ]; then
		if $PERL_EXISTS; then
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT java -mx${MEMLIMIT}k $JAVA_POLICY $MAINFILENAME"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "java -mx${MEMLIMIT}k $JAVA_POLICY $MAINFILENAME"
		fi
		EXITCODE=$?
		if grep -iq -m 1 "Too small initial heap" out || grep -q -m 1 "java.lang.OutOfMemoryError" err; then
			shj_log "Memory Limit Exceeded"
			echo "<span class=\"shj_o\">Memory Limit Exceeded</span>" >>$PROBLEMPATH/user/$UN/result.html
			continue
		fi
		if grep -q -m 1 "Exception in" err; then # show Exception
			...
			if $DISPLAY_JAVA_EXCEPTION_ON && grep -q -m 1 "^$javaexceptionname\$" ../java_exceptions_list; then
				echo "<span class=\"shj_o\">Runtime Error ($javaexceptionname)</span>" >>$PROBLEMPATH/user/$UN/result.html
			else
				echo "<span class=\"shj_o\">Runtime Error</span>" >>$PROBLEMPATH/user/$UN/result.html
			fi
			continue
		fi
	elif [ "$EXT" = "c" ] || [ "$EXT" = "cpp" ]; then
		#$TIMEOUT ./$FILENAME <$PROBLEMPATH/$i.in >out 2>/dev/null
		if $SANDBOX_ON; then
			#LD_PRELOAD=./EasySandbox.so ./$FILENAME <$PROBLEMPATH/$i.in >out 2>/dev/null
			if $PERL_EXISTS; then
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "./timeout --just-kill --sandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT ./$EXEFILE"
			else
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "LD_PRELOAD=./EasySandbox.so ./$EXEFILE"
			fi
			EXITCODE=$?
			# remove <<entering SECCOMP mode>> from beginning of output:
			tail -n +2 out >thetemp && mv thetemp out
		else
			#./$FILENAME <$PROBLEMPATH/$i.in >out 2>/dev/null
			if $PERL_EXISTS; then
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT ./$EXEFILE"
			else
				./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "./$EXEFILE"
			fi
			EXITCODE=$?
		fi
		...
		if $PERL_EXISTS; then
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT python2 -O $FILENAME.py"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "python2 -O $FILENAME.py"
		fi
		EXITCODE=$?

	elif [ "$EXT" = "py3" ]; then
		if $PERL_EXISTS; then
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "./timeout --just-kill -nosandbox -l $OUTLIMIT -t $TIMELIMIT -m $MEMLIMIT python3 -O $FILENAME.py"
		else
			./runcode.sh $EXT $MEMLIMIT $TIMELIMIT $TIMELIMITINT $PROBLEMPATH/$i.in "python3 -O $FILENAME.py"
		fi
		EXITCODE=$?
	...
	if ! grep -q "FINISHED" err; then
		if grep -q "SHJ_TIME" err; then
			...
			echo "<span class=\"shj_o\">Time Limit Exceeded</span>" >>$PROBLEMPATH/user/$UN/result.html
			continue
			...
			shj_log "Hang Up"
			echo "<span class=\"shj_o\">Process hanged up</span>" >>$PROBLEMPATH/user/$UN/result.html
			continue
		elif grep -q "SHJ_SIGNAL" err; then
			shj_log "Killed by a signal"
			echo "<span class=\"shj_o\">Killed by a signal</span>" >>$PROBLEMPATH/user/$UN/result.html
			continue
		elif grep -q "SHJ_OUTSIZE" err; then
			shj_log "Output Size Limit Exceeded"
			echo "<span class=\"shj_o\">Output Size Limit Exceeded</span>" >>$PROBLEMPATH/user/$UN/result.html
			continue
		fi
	else
	...
	if [ $EXITCODE -eq 137 ]; then
		...
		#echo "<span style='color: orange;'>Time Limit Exceeded</span>" >>$PROBLEMPATH/user/$UN/result.html
		shj_log "Killed"
		echo "<span class=\"shj_o\">Killed</span>" >>$PROBLEMPATH/user/$UN/result.html
		continue
	fi
	...
	if [ -f shj_tester ]; then
		./shj_tester $PROBLEMPATH/$i.in $PROBLEMPATH/$i.sol out
		EC=$?
		if [ $EC -eq 0 ]; then
			ACCEPTED=true
		fi
	else
		cp $PROBLEMPATH/$i.sol correctout
	...
	if $ACCEPTED; then
		shj_log "ACCEPTED"
		echo "<span class=\"shj_g\">ACCEPT</span>" >>$PROBLEMPATH/user/$UN/result.html
		((PASSEDTESTS=PASSEDTESTS+1))
	else
		shj_log "WRONG"
		echo "<span class=\"shj_r\">WRONG</span>" >>$PROBLEMPATH/user/$UN/result.html
	fi
done
...
# show place of exception. So I commented following lines:
	...
	#	echo -e "\n<span class=\"shj_b\">Last Java Exception:</span>" >>$PROBLEMPATH/user/$UN/result.html
	#	echo -e "$javaexceptionname\n$javaexceptionplace" >>$PROBLEMPATH/user/$UN/result.html
	#fi
